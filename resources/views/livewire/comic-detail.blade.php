        ...
        <div>
            ...
            <div class="mt-3 flex items-center">
                <div class="text-2xl text-gray-500 dark:text-white/80">
                    {{ sprintf('%.1f', round($this->comic->average_rating, 1)) }}
                </div>
                <div class="mx-2">
                    <div class="flex items-center">
                        @foreach (range(1, 5) as $rating)
                            <svg class="h-5 w-5 flex-shrink-0 {{ $rating <= floor($this->comic->average_rating) ? 'text-yellow-400': 'text-gray-300' }}" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" data-slot="icon">
