                    ...
                    <flux:rows>
                        @foreach ($records as $record)
                            <flux:row :key="$record->id">
                                ...
                                <flux:cell>
                                    @if ($record->chapter)
                                        <a href="{{ $record->chapter->url() }}" class="text-orange-700 dark:text-amber-500 hover:underline underline-offset-4">{{ $record->chapter->title }}</a>
                                    @else
                                        -
                                    @endif
                                </flux:cell>
                                <flux:cell>
                                    <a href="{{ $record->comic->recentChapterUrl() }}" class="text-orange-700 dark:text-amber-500 hover:underline underline-offset-4">{{ $record->comic->recentChapterTitle() }}</a>
                                </flux:cell>
