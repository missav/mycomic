...
Route::get('/cn/{path?}', \App\Http\Controllers\RedirectBaseLocale::class)->where('path', '.*');

Route::name('zh.')->middleware(SetLocale::class . ':zh')->prefix('/zh')->group(function () {
    require base_path('routes/app.php');
});

Route::name('cn.')->middleware(SetLocale::class . ':cn')->group(function () {
    require base_path('routes/app.php');
});
